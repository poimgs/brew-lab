...
import type { CoffeeReference, ReferenceBrew, CoffeeGoalSummary } from '@/api/coffees';
...
describe('ReferenceSidebar', () => {
  ...
  const createMockBrew = (overrides: Partial<ReferenceBrew> = {}): ReferenceBrew => ({
    id: 'exp-123',
  ...
  describe('empty reference (no brews)', () => {
    it('shows empty state message when no brews exist', () => {
  ...
  describe('brew display', () => {
    ...
    it('displays latest brew indicator when is_best is false', () => {
      const reference = createMockReference({
        brew: createMockBrew({ is_best: false }),
      });
    ...
    it('omits parameters that are null', () => {
      const reference = createMockReference({
        ...
        goals: null, // Remove goals to test brew section only
      });
  ...
  describe('goals display', () => {
    ...
    it('displays goal values', () => {
      // Use a reference with only goals (no brew) to avoid duplicate values
      const reference: CoffeeReference = {
        brew: null,
        goals: createMockGoals(),
      };
      ...
      // Goals values - since no brew, these are unique
    ...
    it('shows only goals when no brew exists', () => {
      const reference: CoffeeReference = {
        brew: null,
        goals: createMockGoals(),
      };
  ...
  describe('interactions', () => {
    it('calls onCopyParameters when copy button is clicked', async () => {
      ...
      expect(mockOnCopyParameters).toHaveBeenCalledWith(reference.brew);
    });
  ...
  describe('change reference button', () => {
    it('shows Change button when onChangeReference is provided and brew exists', () => {
      const reference = createMockReference();
    ...
    it('does not show Change button when no brew exists', () => {
      const reference: CoffeeReference = {
        brew: null,
        goals: createMockGoals(),
      };
  ...
  describe('edge cases', () => {
    it('handles brew without filter paper', () => {
      const reference = createMockReference({
        brew: createMockBrew({
