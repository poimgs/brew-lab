...
interface BrewDetailModalBaseProps {
  onClose: () => void
}

interface BrewDetailModalInteractiveProps extends BrewDetailModalBaseProps {
  brewId: string
...
interface BrewDetailModalReadOnlyProps extends BrewDetailModalBaseProps {
  brew: Brew
  brewId?: never
  referenceBrewId?: never
  onMutate?: never
}

type BrewDetailModalProps =
  | BrewDetailModalInteractiveProps
  | BrewDetailModalReadOnlyProps

export function BrewDetailModal(props: BrewDetailModalProps) {
  const { onClose } = props
  const isReadOnly = !!props.brew

  const navigate = useNavigate()
  const [fetchedBrew, setFetchedBrew] = useState<Brew | null>(null)
  ...
  const [currentRefId, setCurrentRefId] = useState(
    props.referenceBrewId ?? null
  )

  const brew = isReadOnly ? props.brew : fetchedBrew
  const isStarred = !isReadOnly && currentRefId === props.brewId

  const fetchBrew = useCallback(async () => {
    if (isReadOnly) return
    try {
      setLoadError(null)
      const data = await getBrew(props.brewId!)
      setFetchedBrew(data)
    } catch {
  ...
  }, [isReadOnly, props.brewId])

  useEffect(() => {
    if (!isReadOnly) fetchBrew()
  }, [isReadOnly, fetchBrew])
  ...
  return (
    <>
      ...
      {showDeleteDialog && brew && !isReadOnly && (
        <DeleteBrewDialog
          brewId={brew.id}
