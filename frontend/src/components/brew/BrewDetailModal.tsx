...
interface BrewDetailModalProps {
  ...
  onRefresh?: () => void;
  /** List of brew IDs for prev/next navigation */
  brewIds?: string[];
  /** Called when navigating to a different brew */
...
export default function BrewDetailModal({
  ...
  }, [open, brewId, fetchBrew]);

  // Prev/Next navigation
  const currentIndex = brewIds && brewId
    ? brewIds.indexOf(brewId)
    : -1;
  const hasPrev = currentIndex > 0;
  const hasNext = brewIds ? currentIndex < brewIds.length - 1 && currentIndex >= 0 : false;

  const handlePrev = () => {
    if (hasPrev && brewIds && onNavigate) {
      onNavigate(brewIds[currentIndex - 1]);
    }
  };

  const handleNext = () => {
    if (hasNext && brewIds && onNavigate) {
      onNavigate(brewIds[currentIndex + 1]);
    }
  };
  ...
  const handleCopy = async () => {
    ...
    setIsCopying(true);
    try {
      const newBrew = await copyBrew(brewId);
      onOpenChange(false);
      navigate(`/brews/${newBrew.id}/edit`);
    } catch {
  ...
  const handleMarkAsReference = async () => {
    ...
    try {
      await setBestBrew(brew.coffee_id, brew.id);
      onRefresh?.();
    } catch {
        ...
        <DialogContent className="sm:max-w-[600px] max-h-[85vh] flex flex-col">
          {loading ? (
            ...
            <div className="py-8 text-center">
              <p className="text-destructive">{error || 'Brew not found'}</p>
            </div>
          ) : (
            <>
              <DialogHeader>
