...
const variableLabels: Record<string, string> = {
  ...
  cleanliness_intensity: 'Clean',
  complexity_intensity: 'Complex',
...
export default function AnalyzeView({
  ...
  const handleCellClick = async (input: string, outcome: string) => {
    ...
    try {
      const detail = await analyzeBrewsDetail(
        brewIds,
        input,
        outcome
      );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-semibold">Analyze Brews</h2>
          <p className="text-muted-foreground">
            Analyzing {result.brew_count} brews for correlations
          </p>
        </div>
        ...
        <DialogContent className="max-w-2xl">
          ...
          {detailData && !detailLoading && (
            <div className="space-y-4">
              ...
              {detailData.brews.length > 0 && (
                <div>
                  <h4 className="text-sm font-medium mb-2">Contributing Brews</h4>
                  <div className="max-h-32 overflow-y-auto space-y-1">
                    {detailData.brews.slice(0, 10).map((exp) => (
                      <div
                        key={exp.id}
                    ...
                    {detailData.brews.length > 10 && (
                      <p className="text-sm text-muted-foreground">
                        ...and {detailData.brews.length - 10} more
                      </p>
                    )}
                  </div>
                </div>
