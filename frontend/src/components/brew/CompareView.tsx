...
import { type CompareResponse, type DeltaInfo } from '@/api/brews';
...
interface CompareRow {
  label: string;
  field: string;
  getValue: (exp: CompareResponse['brews'][0]) => unknown;
  unit?: string;
...
export default function CompareView({ result, onClose }: CompareViewProps) {
  ...
  const renderSectionHeader = (title: string) => (
    <tr className="bg-muted/50">
      <td colSpan={brews.length + 2} className="px-4 py-2 font-medium text-sm">
        {title}
      </td>
    </tr>
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-semibold">Compare Brews</h2>
          <p className="text-muted-foreground">
            Comparing {brews.length} brews side by side
          </p>
        </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-4" style={{ gridTemplateColumns: `repeat(${brews.length}, 1fr)` }}>
          {brews.map((exp, i) => (
