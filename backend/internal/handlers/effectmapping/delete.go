...
func NewDeleteHandler(svc *effectmapping.EffectMappingService) *DeleteHandler {
...
func (h *DeleteHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	err = h.svc.Delete(r.Context(), userID, mappingID)
	if err != nil {
		if errors.Is(err, repository.ErrEffectMappingNotFound) {
			response.NotFound(w, "effect mapping not found")
			return
		}
		response.InternalServerError(w, "failed to delete effect mapping")
		return
	}
