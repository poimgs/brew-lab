...
func NewCreateHandler(svc *effectmapping.EffectMappingService, validate *validator.Validate) *CreateHandler {
...
func (h *CreateHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	var input models.CreateEffectMappingInput
	...
	created, err := h.svc.Create(r.Context(), userID, &input)
	if err != nil {
		switch {
		case errors.Is(err, effectmapping.ErrNameRequired):
			response.BadRequest(w, "name is required")
		case errors.Is(err, effectmapping.ErrVariableRequired):
			response.BadRequest(w, "variable is required")
		case errors.Is(err, effectmapping.ErrInvalidVariable):
			response.BadRequest(w, "invalid input variable")
		case errors.Is(err, effectmapping.ErrDirectionRequired):
			response.BadRequest(w, "direction is required")
		...
		default:
			response.InternalServerError(w, "failed to create effect mapping")
		}
		return
	}
