...
func NewGetHandler(experimentSvc *experiment.ExperimentService) *GetHandler {
	return &GetHandler{experimentSvc: experimentSvc}
}
...
func (h *GetHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	exp, err := h.experimentSvc.GetByID(r.Context(), userID, experimentID)
	if err != nil {
		if errors.Is(err, repository.ErrExperimentNotFound) {
			response.NotFound(w, "experiment not found")
			return
		}
		response.InternalServerError(w, "failed to get experiment")
		return
	}
