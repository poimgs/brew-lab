...
type TryMappingHandler struct {
	svc      *recommendationservice.RecommendationService
	validate *validator.Validate
}

func NewTryMappingHandler(svc *recommendationservice.RecommendationService, validate *validator.Validate) *TryMappingHandler {
	return &TryMappingHandler{svc: svc, validate: validate}
}

func (h *TryMappingHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	var input models.TryMappingInput
	...
	newExperiment, err := h.svc.TryMapping(r.Context(), userID, experimentID, &input)
	if err != nil {
		...
		response.InternalServerError(w, "failed to create experiment from mapping")
		return
	}

	response.Created(w, newExperiment)
}
