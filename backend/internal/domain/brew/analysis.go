...
func GenerateInsights(correlations map[string]map[string]*CorrelationResult) []Insight {
	...
	for i := 0; i < len(entries) && i < 3; i++ {
		...
		insight := Insight{
			...
			Outcome: e.outcome,
			R:       e.result.R,
			Message: inputLabel + " " + strength + " affects " + outcomeLabel + " (" + direction + ", r=" + formatFloat(e.result.R) + ") in these brews.",
		}
...
// ExtractInputValue extracts an input variable value from an brew
func ExtractInputValue(exp *Brew, variable string) *float64 {
	switch variable {
	case "coffee_weight":
...
// ExtractOutcomeValue extracts an outcome variable value from an brew
func ExtractOutcomeValue(exp *Brew, variable string) *float64 {
	switch variable {
	case "overall_score":
...
// CalculateCorrelations computes the correlation matrix for a set of brews
func CalculateCorrelations(brews []Brew, minSamples int) (map[string]map[string]*CorrelationResult, []Warning) {
			...
			if len(xVals) < minSamples {
				...
				if len(xVals) > 0 && len(xVals) < minSamples && !warnedFields[outcomeVar] {
					warnings = append(warnings, Warning{
						...
						Field:   outcomeVar,
						N:       len(xVals),
						Message: "Only " + intToStr(len(xVals)) + " brews have " + formatVariableName(outcomeVar) + " data",
					})
...
// CalculateDelta computes delta information for a field across brews
